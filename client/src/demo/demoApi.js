...
// index.js) - PCDEK/ADEK were never ported to this demo build, so they're
// left out here too rather than accepted at creation and then failing (or
// silently falling back to round robin) the moment "Generate Fixtures" is
// clicked. Killer Classic/Cards Killer ARE fully ported (see logic/killer.js
// and the killer start/shot/undo/reset methods below), so they're included.
...
const SCHEDULING_TYPES = ['round_robin_single', 'round_robin_double', 'knockout_single_elim', 'knockout_double_elim', FREE_PLAY, ...KILLER_TYPES];
...
export const demoApi = {
  ...
  recordFrame: op((fixtureId, winnerPlayerId, method, breaker) => {
    ...
    // Optional BND (Break and Dish) / RND (Reverse Break and Dish) tag -
    // mirrors the server route.
    if (method !== undefined && method !== null && method !== 'bnd' && method !== 'rnd') {
      throw new ApiError(400, "method must be 'bnd', 'rnd', or omitted");
    }
    // Optional record of who won the lag and broke this frame - mirrors the
    // server route. Purely informational, doesn't affect scoring.
  ...
  recordLegFrame: op((fixtureId, legNumber, winnerPlayerId, method, breaker) => {
    ...
    // See the matching recordFrame mock above for what `method` (BND/RND) does.
    if (method !== undefined && method !== null && method !== 'bnd' && method !== 'rnd') {
      throw new ApiError(400, "method must be 'bnd', 'rnd', or omitted");
    }
    // See the matching recordFrame mock above for what `breaker` does.
    if (breaker !== undefined && breaker !== null && typeof breaker !== 'string') {
      throw new ApiError(400, 'breaker must be a player id, or omitted');
    }
